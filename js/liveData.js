 ...
 * The same shape nflTeams.js builds synthetically, so it can be dropped
 * straight in as an override.
 * @param {Array<Object>} rows fsnv2_nfl_schedule output
 */
export function buildLiveSlate(rows) {
  ...
  for (const row of rows) {
    ...
    const away = teamAbbr(row?.away_team);
